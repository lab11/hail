...
pub unsafe fn reset_handler() {
    ...
    //
    // UART for AT commands to Wi-Fi radio
    //

    // First, put the correct USART that is connected to the external
    // pins on the Hail board (and then to the Wi-Fi shield) in UART mode.
    sam4l::usart::USART2.set_mode(sam4l::usart::UsartMode::Uart);

    // Now we need to add a layer that implements the receive until
    ...
    // Start AT UART.
    at_driver.initialize(hil::uart::UARTParameters {
            baud_rate: 115200,
